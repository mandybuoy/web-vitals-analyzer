...
- INP Element Risk Analysis: identify up to 8 interactive HTML elements (buttons, links, inputs, forms) from the HTML source and PSI data that are at risk of poor INP. For each, provide: CSS selector, visible label, interaction type (Click/Type/Keypress), risk level (high/medium/low), contributing script URLs, reason for the risk level, and a specific fix recommendation. Risk tiers: "high" = element directly impacted by a long task >100ms or render-blocking script; "medium" = shares main thread with moderate blocking 50-100ms; "low" = minimal blocking risk. Reference specific script URLs from long-tasks or bootup-time. Output as "inp_element_risks" array in the response.
