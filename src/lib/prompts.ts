...
- INP Element Risk Analysis: identify up to 8 interactive HTML elements (buttons, links, inputs, forms) from the HTML source and PSI data that are at risk of poor INP. For each element provide: CSS selector, visible label, interaction type (Click/Type/Keypress), risk level (high/medium/low), estimated_inp_ms (estimated total INP for this element in ms), input_delay_ms, processing_ms, presentation_delay_ms (the 3 phases must sum to estimated_inp_ms), contributing script URLs, reason for the risk level, and a specific fix recommendation. Estimation rules: the highest-risk element's estimated_inp_ms should be close to the aggregate INP value. Lower-risk elements should be proportionally less. Base estimates on long-task durations for contributing scripts and the element's proximity to heavy JS. Risk tiers: "high" = estimated_inp_ms > 200ms or directly impacted by a long task >100ms; "medium" = 100-200ms or moderate blocking 50-100ms; "low" = <100ms, minimal blocking. Reference specific script URLs from long-tasks or bootup-time. Output as "inp_element_risks" array in the response.
