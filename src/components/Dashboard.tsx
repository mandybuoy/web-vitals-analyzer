...
export default function Dashboard({ mobile, desktop, analysis, isAnalyzing }: DashboardProps) {
  ...
  return (
    <div className="mt-10 animate-fade-up" style={{ animationFillMode: 'forwards' }}>
      {/* URL & timestamp */}
      <div className="mb-6">
        <p className="text-[11px] text-vecton-dark/60 font-mono truncate">{activeResult.url}</p>
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
            {activeResult.fieldData.lcp && (
              <div className="p-4 rounded-lg bg-white/50 border border-vecton-dark/10">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-vecton-dark/50 uppercase tracking-wider">LCP</span>
                ...
                <p className="text-xl text-vecton-dark font-mono">{(activeResult.fieldData.lcp.percentile / 1000).toFixed(2)}s</p>
                <p className="text-[10px] text-vecton-dark/40 mt-1">75th percentile</p>
              </div>
            )}
            {activeResult.fieldData.inp && (
              <div className="p-4 rounded-lg bg-white/50 border border-vecton-dark/10">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-vecton-dark/50 uppercase tracking-wider">INP</span>
                  <span className={`text-xs px-2 py-0.5 rounded
                ...
                <p className="text-xl text-vecton-dark font-mono">{activeResult.fieldData.inp.percentile}ms</p>
                <p className="text-[10px] text-vecton-dark/40 mt-1">75th percentile</p>
              </div>
            )}
            {activeResult.fieldData.cls && (
              <div className="p-4 rounded-lg bg-white/50 border border-vecton-dark/10">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-vecton-dark/50 uppercase tracking-wider">CLS</span>
                  <span className={`text-xs px-2 py-0.5 rounded
                ...
                <p className="text-xl text-vecton-dark font-mono">{(activeResult.fieldData.cls.percentile / 100).toFixed(3)}</p>
                <p className="text-[10px] text-vecton-dark/40 mt-1">75th percentile</p>
              </div>
            )}
            {activeResult.fieldData.fcp && (
              <div className="p-4 rounded-lg bg-white/50 border border-vecton-dark/10">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-vecton-dark/50 uppercase tracking-wider">FCP</span>
                  <span className={`text-xs px-2 py-0.5 rounded
                ...
                <p className="text-xl text-vecton-dark font-mono">{(activeResult.fieldData.fcp.percentile / 1000).toFixed(2)}s</p>
                <p className="text-[10px] text-vecton-dark/40 mt-1">75th percentile</p>
              </div>
            )}
          </div>
      ...
      <div className="mb-8">
        ...
        >
          ...
          <svg
            className={`w-3 h-3 text-vecton-dark/50 transition-transform ${showLabData ? 'rotate-180' : ''}`}
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
        ...
        <div className="mb-8">
          ...
          <div className="space-y-2">
            {activeResult.opportunities.map((opp) => (
              <div
                key={opp.id}
                className="flex items-center justify-between p-3 rounded-lg bg-white/50 border border-vecton-dark/10"
              >
                <p className="text-sm text-vecton-dark/70">{opp.title}</p>
                {opp.savings && (
